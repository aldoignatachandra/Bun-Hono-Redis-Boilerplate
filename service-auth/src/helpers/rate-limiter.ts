...
// In-memory bucket store keyed by method + path + identity.
const buckets = new Map<string, Bucket>();
let lastCleanup = 0;

// Cleanup runs periodically to avoid unbounded growth.
...
// Token bucket check with lazy refill and consumption.
export const checkAndConsume = (
  ...
  // New bucket or new policy for the same key.
  if (
    !existing ||
  ...
  // Calculate retry time based on token refill pace.
